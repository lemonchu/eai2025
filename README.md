...
- `gym_so101_scene/layout.py` encodes the centimeter measurements from Figure 2 (table 60×60 cm, boundary width 1.8 cm, updated robot pads, camera pose at (31.6 cm, 26.0 cm, 40.7 cm)).
- `TableLayout` converts those values to meters, spawns the refreshed black boundary lines, and positions both robot-base markers + cameras in Sapien when the env resets.
